...
async def create_todo_item(todo: TodoItem):
    if todo.id in todo_store:
        raise HTTPException(status_code=400, detail="Todo item with this ID already exists")
    todo_store[todo.id] = todo
    return todo

@app.get("/todos/", response_model=List[TodoItem])
async def get_todo_items() -> List[TodoItem]:
    return [{"id": 1, "description": "Example description", "completed": False, "secretData": "This is secret"}, 
            {"id": 2, "title": "Example 2", "description": "Example description 2", "completed": True, "secretData": "This is also secret"}]
